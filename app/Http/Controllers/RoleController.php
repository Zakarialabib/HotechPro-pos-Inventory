...
class RoleController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect('role')->with('message', 'Données à jour et enregistrées avec succès');
    }
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('activity')){
           $permission = Permission::firstOrCreate(['name' => 'activity']);
           if(!$role->hasPermissionTo('activity')){
                   $role->givePermissionTo($permission);
               }
            }
        else
            $role->revokePermissionTo('activity');

        if($request->has('resume')){
            $permission = Permission::firstOrCreate(['name' => 'resume']);
                if(!$role->hasPermissionTo('resume')){
                        $role->givePermissionTo($permission);
                    }
                 }
        else
                 $role->revokePermissionTo('resume');
